    ...
    return (
        ...
        <div className="w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            ...
            {data.map((sub: any | RecentSubmission) => {
                return (
                    <a href={`https://leetcode.com/submissions/detail/${sub.id}/`} target="_blank" key={sub.id}  aria-current="true" className="grid grid-cols-2 justify-between items-center  w-full px-4 py-2 dark:text-white  border-b border-gray-200 cursor-pointer dark:bg-gray-800 dark:border-gray-600">
                        <div>
